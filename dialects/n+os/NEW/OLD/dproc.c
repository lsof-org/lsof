...
static char *rcsid = "$Id: dproc.c,v 1.11 2000/11/03 18:05:31 abe Exp $";
#endif
...
gather_proc_info()
{
	...
	struct task {			/* (Should come from <kern/task.h>.) */
		caddr_t d1[2+SIMPLE_LOCK_SIZE];
		caddr_t map;
		caddr_t d2[9+SIMPLE_LOCK_SIZE];
...
#if STEPV<40
    /*
	 * Kernel structures seem to be all wrong for this on
	 * OPENSTEP 4.x.  *May* work accidentally on i386, but
	 * definitely doesn't work on m68k.  Does work on NEXTSTEP/i386.
	 */
	    if (t.map)
		process_map(t.map);
#endif
