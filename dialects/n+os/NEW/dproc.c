	...
	caddr_t map;
{
	...
	struct vm_map {		     /* (Should come from <vm/vm_map.h>.) */
		...
		caddr_t pmap;
		unsigned int size;
		boolean_t is_main_map; /* Darwin header has this as ref_count, but... */
	} vmm;
	struct vm_object {	     /* (Should come from <vm/vm_object.h>.) */
		caddr_t d1[SIMPLE_LOCK_SIZE + 4];
		unsigned int size;
...
#endif
    /* Couldn't find this in Darwin, but seems to be same as NEXTSTEP's */
