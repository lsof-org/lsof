...
_PROTOTYPE(static int make_sock,(struct pst_fileinfo2 *f,
 ...
 *
 * Return: SOCKHAS_* flags of what socket contains  (See dlsof.h)
 */
...
/*
 * Copy stream size information.  Note that qlen is missing from pst_stream
...
/*
 * Return flags for what is available in the socket.
 */
	return(av);
}
#endif	/* defined(PS_STR_XPORT_DATA) */
 ...
 */
	...
	if (av & SOCKHAS_QLEN) {
	    ...
	    Lf->lts.qlens = (unsigned char)1;
	}
	if (av & SOCKHAS_QLIMIT) {
	    Lf->lts.qlim = (unsigned int)s->pst_qlimit;
	    Lf->lts.qlims = (unsigned char)1;
	}
#endif	/* defined(HASSOOPT) */
 ...
 */
	...
	case PS_AF_INET6:
		...
		Lf->lts.rws = (unsigned char)1;
	    }
	    if (av & SOCKHAS_SWND) {
		Lf->lts.ww = (unsigned long)s->pst_swnd;
		Lf->lts.wws = (unsigned char)1;
	    }
	}
#endif	/* defined(HASTCPTPIW) */
 ...
 */
	 ...
	 */
	    av = make_sock(f, &s[hx], &sck);
	    (void) process_socket(f, &sck, av);
	    return;
	}
#endif	/* defined(PS_STR_XPORT_DATA) */
