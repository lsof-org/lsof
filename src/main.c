...
char *argv[];
{
    ...
    if ((Pn = strrchr(argv[0], '/')))
        ...
        Pn = argv[0];
    lsof_output_stream(ctx, stderr, Pn, 0);
...
#if defined(HASEOPT)
        case 'e':
            if (lsof_exempt_fs(ctx, GOv, ((GOp == '+') ? 1 : 0)))
                err = 1;
            break;
#endif /* defined(HASEOPT) */
