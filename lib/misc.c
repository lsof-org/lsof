/*
 * misc.c - common miscellaneous functions for liblsof
 */
...
int doinchild(struct lsof_context *ctx, int (*fn)(), /* function to perform */
              ...
              int rbln) /* response buffer length */
{
     ...
     */
    if (!Fovhd && rbln > MAXPATHLEN && ctx->err) {
        (void)fprintf(ctx->err,
    ...
    if (!Fovhd) {
        if (setjmp(Jmp_buf)) {
            ...
            if (pipe(Pipes) < 0 || pipe(&Pipes[2]) < 0) {
                if (ctx->err)
                    (void)fprintf(ctx->err, "%s: can't open pipes: %s\n", Pn,
                                  strerror(errno));
                Error(ctx);
            }
            /*
            ...
            if (Cpid < 0) {
                if (ctx->err)
                    (void)fprintf(ctx->err, "%s: can't fork: %s\n", Pn,
                                  strerror(errno));
                Error(ctx);
            }
            (void)close(Pipes[0]);
...
void enter_dev_ch(struct lsof_context *ctx, char *m) {
    ...
    if (!(mp = mkstrcpy(m, (MALLOC_S *)NULL))) {
        if (ctx->err) {
            (void)fprintf(ctx->err, "%s: no more dev_ch space at PID %d: \n",
                          Pn, Lp->pid);
            safestrprt(m, ctx->err, 1);
        }
        Error(ctx);
    }
    if (Lf->dev_ch)
                   ...
                   int nr)   /* state number */
{
    ...
    if (!strcmp(ty, "TCP"))
        ...
        if (ctx->err)
            (void)fprintf(ctx->err, "%s: unknown type for enter_IPstate: %s\n",
                          Pn, ty);
        Error(ctx);
        return;
    }
    ...
    if (strlen(nm) < 1) {
        if (ctx->err)
            (void)fprintf(ctx->err, "%s: bad %s name (\"%s\"), number=%d\n", Pn,
                          ty, nm, nr);
        Error(ctx);
        return;
    }
    ...
    if (!(cp = mkstrcpy(nm, (MALLOC_S *)NULL))) {
        if (ctx->err)
            (void)fprintf(ctx->err, "%s: enter_IPstate(): no %s space for %s\n",
                          Pn, ty, nm);
        Error(ctx);
        return;
    }
    ...
    if (nn > al) {
        ...
        if (tx) {
            ...
            no_IP_space:
                ...
                if (ctx->err)
                    (void)fprintf(ctx->err, "%s: no %s state space\n", Pn, ty);
                Error(ctx);
                goto cleanup;
            }
            UdpSt = temp;
    ...
    if (tx) {
        ...
        dup_IP_state:
            ...
            if (ctx->err)
                (void)fprintf(
                    ctx->err, "%s: duplicate %s state %d (already %s): %s\n",
                    Pn, ty, nr,
                    tx ? UdpSt[nr + UdpStOff] : TcpSt[nr + TcpStOff], nm);
            Error(ctx);
            goto cleanup;
        }
...
void enter_nm(struct lsof_context *ctx, char *m) {
    ...
    if (!(mp = mkstrcpy(m, (MALLOC_S *)NULL))) {
        if (ctx->err) {
            (void)fprintf(ctx->err, "%s: no more nm space at PID %d for: ", Pn,
                          Lp->pid);
            safestrprt(m, ctx->err, 1);
        }
        Error(ctx);
...
int is_nw_addr(struct lsof_context *ctx,
...
int is_readable(struct lsof_context *ctx, char *path, /* file path */
...
int readstdata(struct lsof_context *ctx,
...
int readsthead(struct lsof_context *ctx,
...
int readstidnm(struct lsof_context *ctx,
...
int readstmin(struct lsof_context *ctx,
...
int readstqinit(struct lsof_context *ctx,
...
void stkdir(struct lsof_context *ctx, char *p) /* directory path */
{
    ...
    if (!(Dstk[Dstkx] = mkstrcpy(p, (MALLOC_S *)NULL))) {
        if (ctx->err) {
            (void)fprintf(ctx->err, "%s: no space for: ", Pn);
            safestrprt(p, ctx->err, 1);
        }
        Error(ctx);
    }
...
void dropgid(struct lsof_context *ctx) {
    if (!Setuidroot && Setgid) {
        if (setgid(Mygid) < 0) {
            if (ctx->err)
                (void)fprintf(ctx->err, "%s: can't setgid(%d): %s\n", Pn,
                              (int)Mygid, strerror(errno));
            Error(ctx);
        }
        Setgid = 0;
    }
