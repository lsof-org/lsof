...
#        define re_match_2(bufp, string1, size1, string2, size2, pos, regs,    \
                           stop)                                               \
...
#        define re_search_2(bufp, st1, s1, st2, s2, startpos, range, regs,     \
                            stop)                                              \
...
#        else
            printf("/succeed_n to %ld, %d times", (long int)(p1 - start),
                   mcnt2);
#        endif
            break;
...
#        else
            printf("/set_number_at location %ld to %d", (long int)(p1 - start),
                   mcnt2);
#        endif
            break;
...
#    define REG_ECOLLATE_IDX                                                   \
        (REG_BADPAT_IDX + sizeof "Invalid regular expression")
...
#    define REG_ERPAREN_IDX                                                    \
        (REG_ESIZE_IDX + sizeof "Regular expression too big")
...
#    define POP_FAILURE_POINT(str, pat, low_reg, high_reg, regstart, regend,   \
        ...
        {                                                                      \
            ...
            if (1)                                                             \
                ...
                for (this_reg = highest_active_reg; this_reg > high_reg;       \
                     this_reg--) {                                             \
...
static void store_op2 _RE_ARGS((re_opcode_t op, unsigned char *loc, int arg1,
                                int arg2));
static void insert_op1 _RE_ARGS((re_opcode_t op, unsigned char *loc, int arg,
                                 unsigned char *end));
...
static reg_errcode_t compile_range
    _RE_ARGS((unsigned int range_start, const char **p_ptr, const char *pend,
...
#    endif
    ...
    if (bufp->allocated == 0) {
        if (bufp->buffer) { /* If zero allocated, but buffer is non-null, try to
                               realloc enough space.  This loses if buffer's
                               address is bogus, but that is the user's
                               responsibility.  */
    ...
    while (p != pend) {
        ...
        case '*':
...

            {
                ...
                if (many_times_ok) { /* More than one repetition is allowed, so
                                        put in at the end a backward relative
                                        jump from `b' to before the next jump
                                        we're going to put in below (which jumps
                                        from laststart to after this jump).

                                        But if we are at the `*' in the exact
                                        sequence
                                        `.*\n', insert an unconditional jump
                                        backwards to the ., instead of the
                                        beginning of the loop. This way we only
                                        push a failure point once, instead of
        ...
        case '[': {
            ...
            for (;;) {
                ...
                if (c == '-' && !(p - 2 >= pattern && p[-2] == '[') &&
                    !(p - 3 >= pattern && p[-3] == '[' && p[-2] == '^') &&
                    *p != ']') {
                    reg_errcode_t ret = compile_range(range_start, &p, pend,
                                                      translate, syntax, b);
                    if (ret != REG_NOERROR)
                    ...
                    range_start = 0xffffffff;
                         ...
                         p[1] != ']') { /* This handles ranges made up of
                                           characters only.  */
                    reg_errcode_t ret;
...
#    endif
        ...
        case '\\':
            ...
            handle_close:
                if (fixup_alt_jump) { /* Push a dummy failure point at the end
                                         of the alternative for a possible
                                         future `pop_failure_jump' to pop.  See
                                         comments at `push_dummy_failure' in
                                         `re_match_2'.  */
                ...
                assert(compile_stack.avail != 0);
                {
                    ...
                    fixup_alt_jump =
                        COMPILE_STACK_TOP.fixup_alt_jump
                            ? bufp->buffer + COMPILE_STACK_TOP.fixup_alt_jump -
                                  1
                            : 0;
                    laststart =
...
static reg_errcode_t compile_range(range_start_char, p_ptr, pend, translate,
                                   syntax, b)
...
static int bcmp_translate _RE_ARGS((const char *s1, const char *s2, int len,
                                    char *translate));
...
#    endif
    ...
    /* This is a separate function so that we can force an alloca cleanup
       afterwards.  */
    static int re_match_2_internal(bufp, string1, size1, string2, size2, pos,
                                   regs, stop)
struct re_pattern_buffer *bufp;
const char *string1, *string2;
int size1, size2;
        ...
        succeed_label:
            ...
            if (regs && !bufp->no_sub) {
                ...
                if (bufp->regs_allocated ==
                    REGS_UNALLOCATED) { /* No.  So allocate them with malloc. We
                                           need one extra element beyond
                                           `num_regs' for the
                                           `-1' marker GNU code uses.  */
                    ...
                    bufp->regs_allocated = REGS_REALLOCATE;
                           ...
                           REGS_REALLOCATE) { /* Yes.  If we need more elements
                                                 than were already allocated,
                                                 reallocate them.  If we need
            ...
            DEBUG_PRINT2("EXECUTING maybe_pop_jump %d.\n", mcnt);
            {
                ...
                if (p2 == pend) {
                    ...
                    if ((re_opcode_t)p1[3] == exactn &&
                        ...
                        for (idx = 0; idx < (int)p2[1] && idx < (int)p1[4];
                             idx++)
