...
_PROTOTYPE(extern void process_finfo,
           (struct pst_filedetails * pd, struct pst_fid *opfid,
            struct psfileid *psfid, KA_T na));
_PROTOTYPE(extern void process_socket,
           (struct pst_fileinfo2 * f, struct pst_socket *s));
_PROTOTYPE(extern void process_stream, (struct pst_fileinfo2 * f, int ckscko));
_PROTOTYPE(extern KA_T read_det,
           (struct pst_fid * ki, uint32_t hf, uint32_t lf, uint32_t hn,
...
_PROTOTYPE(extern int read_vxnode,
           (struct vnode * v, struct l_vfs *vfs, dev_t *dev));
