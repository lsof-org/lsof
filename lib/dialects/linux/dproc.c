...
void initialize(struct lsof_context *ctx) {
     ...
     * than just Internet lookups.  (HasNFS is defined during the
     * loading of the local mount table.)
     */
    if (Selinet == 0)
        (void)readmnt(ctx);
}
...
/*
 * make_proc_path(ctx, ) - make a path in a /proc directory
 *
 * entry:
                 ...
                 int ss)         /* *s status -- i.e., SB_* values */
{
     ...
     * and read it/
     */
    if (!(ms = open_proc_stream(ctx, p, "r", &vbuf, &vsz)))
        return;
