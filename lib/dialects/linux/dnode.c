...
endpoint_find(struct lsof_context *ctx, pxinfo_t **pinfo_hash,
              int (*is_acceptable)(struct lsof_context *ctx, pxinfo_t *, int,
                                   struct lfile *),
                            ...
                            * zero) */
{
    ...
    for (cp = ln, n = 0; cp && *cp;) {
        ...
        if (*cp)
            *cp++ = '\0';
        if (n >= ctxd.get_fields_buffer_size) {
            ctxd.get_fields_buffer_size += 32;
            len = (MALLOC_S)(ctxd.get_fields_buffer_size * sizeof(char *));
            if (ctxd.get_fields_buffer)
                ctxd.get_fields_buffer =
                    (char **)realloc((MALLOC_P *)ctxd.get_fields_buffer, len);
            else
        ...
        ctxd.get_fields_buffer[n++] = bp;
    }
    *fr = ctxd.get_fields_buffer;
    return (n);
}
...
void get_locks(struct lsof_context *ctx, char *p) /* /proc lock path */
{
     ...
     * and read it.
     */
    if (!(ls = open_proc_stream(ctx, p, "r", &vbuf, &vsz)))
        return;
...
/*
 * clean_locks() - cleanup lock information
 */

void clean_locks(struct lsof_context *ctx) {
    struct llock *lp, *np;
    int i;
    if (LckH) {
        for (i = 0; i < PIDBUCKS; i++) {
