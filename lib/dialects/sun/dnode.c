...
int CTF_getmem(struct lsof_context *ctx, ctf_file_t *f, /* CTF file handle */
...
void CTF_init(struct lsof_context *ctx, int *i, /* initialization status */
...
static int is_socket(struct lsof_context *ctx,
...
static struct l_dev *finddev(struct lsof_context *ctx, dev_t *dev, /* device */
...
static int idoorkeep(struct lsof_context *ctx,
...
#    else /* solaris<110000 */
        {
             ...
             *	  address);
             *    Get a non-NULL local sockaddr_un and enter it in
             *Namech; Get a non-NULL foreign sockaddr_un and enter it in
             *Namech; Check for matches on sockaddr_un.sun_path names.
             */
        ...
        if (((Ntype = vty) == N_STREAM) && so_st) {
            ...
            if (so_ad[0]) {
                if (sdp) {
                    ...
                    enter_dev_ch(ctx, print_kptr(so_ad[0], (char *)NULL, 0));
                    if (so_ad[1])
...
#    endif /* solaris<20400 */
                ...
                dc = (dl << 16) | dr;
                enter_dev_ch(ctx, print_kptr((KA_T)dc, (char *)NULL, 0));
                devs = 0;
            }
            ...
            if (sdp) {
                ...
                if (Lf->is_com && !Lf->nma) {
                    ...
                    if (!(Lf->nma = (char *)malloc(len))) {
                        (void)fprintf(
                            stderr,
                            "%s: no space for (COMMON): PID %d; FD %d\n", Pn,
                            Lp->pid, Lf->fd_num);
                        Error(ctx);
                    }
...
static int read_cni(struct lsof_context *ctx,
...
static int readinode(struct lsof_context *ctx,
...
static int read_ndn(struct lsof_context *ctx,
...
static void read_mi(struct lsof_context *ctx,
...
static int read_nan(struct lsof_context *ctx,
...
static int read_ncn(struct lsof_context *ctx,
                    KA_T na,          /* containing node's address */
...
static int read_nctfsn(struct lsof_context *ctx,
...
static int read_nfn(struct lsof_context *ctx,
                    KA_T na,            /* containing node's address */
...
static int read_nhn(struct lsof_context *ctx,
                    KA_T na,          /* containing node's address */
...
static int read_nin(struct lsof_context *ctx,
                    KA_T na,         /* containing node's address */
...
static int read_nln(struct lsof_context *ctx,
                    KA_T na,          /* containing node's address */
...
static int read_nnn(struct lsof_context *ctx,
                    KA_T na,             /* containing node's address */
...
static int read_nmn(struct lsof_context *ctx,
                    KA_T na,            /* containing node's address */
...
static int read_npi(struct lsof_context *ctx,
                    KA_T na,          /* containing node's address */
...
static int read_npn(struct lsof_context *ctx,
                    KA_T na,          /* containing node's address */
...
static int read_nprtn(struct lsof_context *ctx,
                      KA_T na,   /* containing node's address */
...
static int read_nrn(struct lsof_context *ctx,
                    KA_T na,         /* containing node's address */
...
static int read_nrn4(struct lsof_context *ctx,
                     KA_T na,          /* containing node's address */
                     KA_T ra,          /* rnode address */
...
static int read_nsdn(struct lsof_context *ctx,
...
static int read_nson(struct lsof_context *ctx,
                     KA_T na,           /* containing node's address */
...
static int read_nsn(struct lsof_context *ctx,
                    KA_T na,         /* containing node's address */
...
static int read_nsti(struct lsof_context *ctx,
...
static int read_ntn(struct lsof_context *ctx,
                    KA_T na,           /* containing node's address */
...
static int read_nvn(struct lsof_context *ctx, KA_T na, /* node's address */
...
static int read_nzn(struct lsof_context *ctx,
                    KA_T na,     /* containing node's address */
...
static int read_nznp(struct lsof_context *ctx,
...
static int read_nzvfs(struct lsof_context *ctx,
                      KA_T nza,     /* containing znode's address */
...
int vop2ty(struct lsof_context *ctx, struct vnode *vp, /* local vnode pointer */
...
static int read_ndvn(struct lsof_context *ctx,
                     KA_T na,             /* containing vnode's address */
