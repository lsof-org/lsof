...
#if defined(HASIPv6)
        Lf->type = (af == AF_INET6) ? LSOF_FILE_IPV6 : LSOF_FILE_IPV4;
...
#    define t_outqsize t_qsize
    ...
    int process_unix_sockstr(v, na) struct vnode *v; /* the stream's vnode */
...
#    else  /* UNIXWAREV>=70103 */
        ...
        /*
         * It's serious if the sizeof(sun_path) in sockaddr_un isn't greater
         * than zero.
         */
        if (up < 0) {
...
#    else  /* UNIXWAREV>=70103 */
        if (((as = (KA_T)ss.remote_addrsz) > 0) &&
            (ka = (KA_T)ss.remote_addr)) {
            if (as > sizeof(la))
                as = (int)sizeof(ra);
...
#    endif /* UNIXWAREV<70103 */
        ...
        /*
         * Start Namech[] with the service type, converted to a name, ala
         * netstat.
         */
        switch (ss.servtype) {
        case T_COTS:
...
#    if UNIXWAREV < 70103
    ...
    static struct sockaddr_un *find_unix_sockaddr_un(ka)
        KA_T ka; /* socket's kernel address */
    {
        ...
        if (!al) {
            ...
            /*
             * The list has been acquired.  Free any excess space pre-allocated
             * to it, then save its address.   Close the stream socket.
             */
            alct = ct;
        ...
        /*
         * Search a previously acquired address list, based on the supplied
         * kernel socket address.  If an entry is found, return a pointer to it,
         * making sure the path it contains is terminated.
         */
        for (i = 0; i < alct; i++) {
