...
struct lsof_context {
    ...
    int logic_and;      /* -a option status */
    int avoid_blocking; /* -b option status */

    /* file descriptors selected with -d */
    struct fd_lst *fd_list;
    int fd_list_ty; /* fd_list[] type: -1 == none
                     *		0 == include
                     *		1 == exclude */

    int endpoint_status; /* -E option status: 0==none, 1==info,
                          * 2==info+files */
