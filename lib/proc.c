/*
 * proc.c - common process and file structure functions for liblsof
 */
 ...
 *	   2 == FD is included
 */
int ck_fd_status(struct lsof_context *ctx,
                 enum lsof_fd_type fd_type, /* file descriptor type */
                 int num)                   /* file descriptor number -- -1 if
                                             * none */
{
...
int is_file_sel(struct lsof_context *ctx,
                               ...
                               */
{
    ...
    if (!FeptE)
        return;
    for (Lf = Lp->file; Lf; Lf = Lf->next) {
        if ((Ntype != N_ANON_INODE) || (Lf->eventfd_id == -1))
            continue;
        pp = (pxinfo_t *)NULL;
        switch (f) {
        case 0:
...
/*
 * clean_lfile() - free local file structure space
 */
void clean_lfile(struct lsof_context *ctx, struct lfile *lf) {
    CLEAN(lf->dev_ch);
    CLEAN(lf->nm);
    CLEAN(lf->nma);
    memset(lf, 0, sizeof(struct lfile));
